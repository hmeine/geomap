    ...
    def points(self, useConstraint = True, t = 1e-7):
        ...
        if not useConstraint:
            ...
            def checkConstraint(z, i, xx, yy, t):
                if i.g2(xx,yy) < t:
                    return False
                gx, gy = i.dx(xx,yy), i.dy(xx,yy)
                return (gx**3 * i.dx3(xx,yy) + \
                        3*gx**2 * gy * i.dxxy(xx,yy) + \
                        3*gx * gy**2 * i.dxyy(xx,yy) + \
                        gy**3 * i.dy3(xx,yy) < 0)
        elif self.m in "laplace":
            def checkConstraint(z, i, xx, yy, t):
                if i.g2(xx,yy) < t:
                    return False
                gx, gy = -z.dx(xx,yy), -z.dy(xx,yy)
                return (gx**3 * i.dx3(xx,yy) + \
                        3*gx**2 * gy * i.dxxy(xx,yy) + \
                        3*gx * gy**2 * i.dxyy(xx,yy) + \
                        gy**3 * i.dy3(xx,yy) < 0)
        else:
            def checkConstraint(z, i, x, y, t):
                if i(x,y) < t:
                    return False
...
def levelEdgesMap(edges):
    ...
    result = Map(nodePositions, edgeTriples, roi.size(),
